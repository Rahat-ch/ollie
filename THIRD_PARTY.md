...
- **Anthropic Claude** via the official `@anthropic-ai/sdk` (MIT) — **planned**, not yet a dependency.
  - Claude Sonnet 5 (`claude-sonnet-5`) for Stories.
  - Claude Opus 5 (`claude-opus-5`) for the Coach, Summary, and Judge.
...
Every npm package in the installed tree, including devDependencies and optionalDependencies, with the licence declared in its `package.json`. The table is generated; run `node scripts/third-party.mjs --write` to refresh it.

<!-- BEGIN:deps -->
Generated by `node scripts/third-party.mjs --write` from `pnpm licenses list --json --long`: 363 packages (dependencies, devDependencies, and optionalDependencies). Do not edit by hand.

Licence counts: MIT 310, Apache-2.0 22, ISC 14, BSD-2-Clause 7, MPL-2.0 3, BSD-3-Clause 2, 0BSD 1, BlueOak-1.0.0 1, CC-BY-4.0 1, CC0-1.0 1, Python-2.0 1.
...
| `zod` | 4.6.1 | MIT | https://github.com/colinhacks/zod |
| `zod-validation-error` | 4.0.2 | MIT | https://github.com/causaly/zod-validation-error |
<!-- END:deps -->

## Audit

Run `pnpm licenses:check` (`node scripts/third-party.mjs --check`) to verify that every dependency is permissively licensed or a documented, reviewed exception; it exits non-zero otherwise. Last run: 2026-09-10.
