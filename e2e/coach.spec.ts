...
const RECORD = {
  ...
  summaries: [9, 8, 7, 6, 5, 4, 3].map(summary),
  awaiting: null,
};
...
test("completing a Session runs the Coach once; with no key on the server the next Session is the Baseline's, the Notebook says so, and the Parent still gets a Summary", async ({ page, baseURL }) => {
  ...
  // One Coach run, and no more however the page is left and come back to. A
  // server with no key answers 503, which is not retried: one call each.
  ...
  expect(record.awaiting).toBeNull();
  ...
  // The raw reason is kept for the developer; the Parent never reads it.
  expect(record.reasons[0]).toContain("ANTHROPIC_API_KEY is not set");
  ...
  const baseline = page.getByTestId("notebook-baseline");
  await expect(baseline).toContainText("Ollie could not reach the Coach after the last Session");
  // The Plan it fell back to, counted from the Plan itself.
  await expect(baseline).toContainText("6 Problems on Partners to 10");
  await expect(baseline).not.toContainText("ANTHROPIC_API_KEY");
  ...
  await expect(summaryCard.getByTestId("summary-evidence").first()).toContainText("first-try correct");
  await expect(summaryCard).toContainText("correct after a Hint");
...
test("a Coach run that a reload interrupts is run again from the home screen, and the Notebook and the Summary arrive", async ({ page, baseURL }) => {
  ...
  // The Session waits on the record while the run is away.
  ...
  await openParentArea(page);
  await expect(page.getByTestId("notebook-baseline")).toContainText("Ollie could not reach the Coach");
  await expect(page.getByTestId("parent-summary")).toHaveCount(1);
});
...
test("the Notebook shows each Hypothesis's evidence as the actual Problems, tappable, and the last seven Summaries survive a reload newest first", async ({ page }) => {
  ...
  await expect(evidence.nth(0)).toContainText("correct after a Hint");
  ...
  await expect(summaries.nth(0).getByTestId("summary-evidence")).toContainText(
    "4 first-try correct · 1 correct after a Hint · 1 Revealed · 0 left unanswered",
  );
