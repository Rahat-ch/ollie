...
describe("storyInputFor", () => {
  it("keys a Unit 3 Problem by the Profile's Theme and the engine's numbers, and nothing for Units 1 and 2", () => {
    ...
    expect(storyInputFor(p, "space")).toEqual({ skill: "result-unknown", structure: "add-to", equation: p.equation, answer: 12, theme: "space", rich: false });
...
describe("the rich Story set once Story Solver is learned", () => {
  ...
  it("keys the Problem into the rich set, so the Learner hears a Story written as a scene", () => {
    expect(storyInputFor(p, "space", true)).toMatchObject({ theme: "space", rich: true });
    expect(storyInputFor(p, "space")).toMatchObject({ rich: false });
  });

  it("reads the rich set from the Pool and never the plain one", () => {
    const plain = addToPool({}, storyInputFor(p, "puppies")!, "plain {{nickname}}");
    expect(pooledStory(plain, p, "puppies", true)).toBeUndefined();
    const rich = addToPool(plain, storyInputFor(p, "puppies", true)!, "rich {{nickname}}");
    expect(pooledStory(rich, p, "puppies", true)).toBe("rich {{nickname}}");
  ...
  it("is the same keys again for the fill script, one set for each", () => {
    ...
    const rich = poolInputs(["puppies"], "default", true);
