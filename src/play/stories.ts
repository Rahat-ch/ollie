 ...
 * other Problem. `rich` is the set the Story Solver Power opens: the same
 * Problem, told as a scene in the Theme, kept apart in the Pool.
 */
export function storyInputFor(problem: Problem, theme: ThemeId, rich = false): PoolInput | null {
  if (getSkill(problem.skill).unit !== 3) return null;
  const { skill, structure, equation, answer } = problem;
  return { skill, structure, equation, answer, theme, rich };
}
...
export function pooledStory(pool: ContentPool, problem: Problem, theme: ThemeId, rich = false): string | undefined {
...
export function poolInputs(themes: readonly ThemeId[], range: SkillRange = "default", rich = false): PoolInput[] {
  return themes.flatMap((theme) =>
    UNIT_3.flatMap((skill) =>
      getSkill(skill).structures.flatMap((structure) =>
        equationsFor(skill, structure, range).map((equation): PoolInput => ({ skill, structure, equation, answer: equation[equation.unknown], theme, rich })),
      ),
    ),
  );
}
