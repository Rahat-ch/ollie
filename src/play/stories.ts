...
/**
 * The Pool key for a Unit 3 Problem in the Profile's Theme; null for any
 * other Problem. The rich set is the one the Story Solver Power opens: the
 * same Problem, told as a scene in the Theme, kept apart in the Pool.
 */
export function storyInputFor(problem: Problem, theme: ThemeId, set: StorySet = "plain"): PoolInput | null {
  if (getSkill(problem.skill).unit !== 3) return null;
  const { skill, structure, equation, answer } = problem;
  return { skill, structure, equation, answer, theme, set };
}
...
export function pooledStory(pool: ContentPool, problem: Problem, theme: ThemeId, set: StorySet = "plain"): string | undefined {
...
/** Every Pool key `pnpm pool` fills: each Theme, each Unit 3 Skill and structure, each equation in the Skill's default (or standard) range, in the plain or the rich set. */
export function poolInputs(themes: readonly ThemeId[], range: SkillRange = "default", set: StorySet = "plain"): PoolInput[] {
  return themes.flatMap((theme) =>
    UNIT_3.flatMap((skill) =>
      getSkill(skill).structures.flatMap((structure) =>
        equationsFor(skill, structure, range).map((equation): PoolInput => ({ skill, structure, equation, answer: equation[equation.unknown], theme, set })),
      ),
    ),
  );
}
