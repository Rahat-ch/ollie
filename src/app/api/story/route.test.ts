...
describe("POST /api/story", () => {
  it("answers a Problem the engine could have set with a Story in placeholder form; the template when there is no key", async () => {
    delete process.env.ANTHROPIC_API_KEY;
    const response = await post(body);
    expect(response.status).toBe(200);
    expect(await response.json()).toEqual({
      text: templateStory({ ...body, nickname: NICKNAME_PLACEHOLDER }),
      source: "template",
    });
  });
