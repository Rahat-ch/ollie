...
import { AVATAR_COLORS, THEMES, type ThemeId } from "@/profile/identity";
import type { Rewards } from "@/rewards/rewards";
import { SHOP_ITEMS, type AvatarItem } from "@/rewards/shop";
...
const THEME_SAYS: Readonly<Record<ThemeId, string>> = {
...
/** What the Avatar wears with just this one Item on. */
const wornWith = (item: AvatarItem): Rewards["worn"] => ({
  hat: item.slot === "hat" ? item.id : null,
  accessory: item.slot === "accessory" ? item.id : null,
  pet: item.slot === "pet" ? item.id : null,
});
...
export function DesignReview() {
  return (
    <main className="min-h-dvh bg-paper px-gutter py-12" data-testid="design-review">
      <h1 className="mb-2 font-display text-display-l font-semibold text-ink">Illustration pass</h1>
          ...
          <div key={item.id} className="mb-6 flex flex-wrap items-center gap-4">
            ...
            {AVATAR_COLORS.map((color) => (
              <div key={color.id} className="flex size-40 items-center justify-center rounded-card bg-paper">
                <Avatar color={color.id} worn={wornWith(item)} size={130} label={`${color.name} Avatar wearing ${item.name}`} />
              </div>
            ))}
          </div>
        ))}
      ...
      <Panel
        ...
        note="One flat plum disc per Power, with the shape the Power puts on the screen cut out of it in paper: a pair of wing beats over two hops on the number line, a ten-frame filled to ten, the magnifying glass, and the open book with its page lines."
      >
        <div className="flex flex-wrap gap-4">
          {POWERS.map((power) => (
