...
Generative: the **Stories** (the two-sentence word problems written in the Learner's Theme around the engine's numbers, with her Nickname in them), the **Coach's Hypotheses and Session Plans**, the **words of the Parent Summary**, and **Ollie's voice audio**.
...
![Session to engine to evidence to Coach to validator to next Session](./architecture.svg)

Two seams carry the whole app.
...
No Anthropic or ElevenLabs key ever reached the machine this was built on. The real Coach, Story writer, Parent Summary, and Judge have never been called; the Content Pool file and `public/voice/` are committed empty; no voice has been designed. Everything that needs them is built, tested against the fake, and resumable behind one command each.
