...
There is one local Profile per browser, no login, and no microphone. Progress, Streak, Coins, and Parent Summaries are stored client-side. The only data sent to a server is what the engine needs to write a Story or a Parent Summary, to plan the next Session, or to voice a line: problem numbers, a Theme, per-session skill results, and — for a spoken line alone — the Nickname. No audio is ever recorded.
